...
static void allow_answer(void *data, Evas_Object * /* obj */, void * /* event_info */) {
...
static int show_popup(struct cert_checker_popup_data *pdp) {
    ALOGD("show_popup()");
    ...
    ret = std::snprintf(tmpBuffer, sizeof(tmpBuffer), messageFormat, pdp->client, pdp->user,
            pdp->privilege);
...
elm_main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        ...
        ALOGE("return ERROR");
        test = 1;
        // retrurn popup_status::EXIT_ERROR;
    }
    ...
    ALOGD("Read bytes : " << count << " (in " << difftime(time_start, time(NULL)) << " seconds)");
    ...
    AskUser::Deserialization::Deserialize(stream, client);
    ALOGD("client : " << client.c_str());
    pdp->client = client.c_str();

    AskUser::Deserialization::Deserialize(stream, user);
    ALOGD("user : " << user.c_str());
    pdp->user = user.c_str();

    AskUser::Deserialization::Deserialize(stream, privilege);
    ALOGD("privilege : " << privilege.c_str());
    pdp->privilege = privilege.c_str();
    }

    if (test) {
    pdp->client = "client";
    pdp->user = "user";
    pdp->privilege = "privilege";
    }

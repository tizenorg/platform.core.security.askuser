 ...
 */
/*
 * @file        src/ui/AskUINotificationBackend.cpp
...
bool AskUINotificationBackend::createUI(const std::string &client, const std::string &user,
    ...
    if (ret != PRVMGR_ERR_NONE) {
        ...
        privilegeDisplayName = const_cast<char *>(privilege.c_str());
    }
    LOGD("privilege_info_get_privilege_display_name: [" << ret << "],"
         " <" << privilegeDisplayName << ">");

    char tmpBuffer[BUFSIZ];
    ret = snprintf(tmpBuffer, sizeof(tmpBuffer), messageFormat, client.c_str(), user.c_str(),
                   privilegeDisplayName);
...
bool AskUINotificationBackend::isDismissing() {
    return m_dismissing;
}

...
{
    if (!m_notiTalker.isInitialized()) {
        ALOGE("Notification talker initialization failed because : %s",
              m_notiTalker.getInitErrorMsg().c_str());
        throw std::runtime_error("Couldn't initialize backend");
    }
}

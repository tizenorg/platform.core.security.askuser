...
int main(int argc, char **argv) {
    ...
    profiler_enable("TEST_CLIENT", "client.profile");

    ret = cynara_configuration_set_cache_size(cynara_config, 10);
    printf("set chache size ret [%d]: %s\n", ret, cystrerr(ret));
    ...
    while (repeats--) {
        ...
        profiler_measure_start("TEST_CLIENT", "cynara_check");
        ret = cynara_check(cynar, client, session, user, privilege);
        profiler_measure_stop("TEST_CLIENT", "cynara_check");
        printf("get ret [%d]: %s\n", ret, cystrerr(ret));
